...
{
    public partial class NextifyCheckedListBox : CheckedListBox, IControl
    {
        public NextifyCheckedListBox()
        {
            InitializeComponent();

            if (!DesignMode)
            {
                Validator = new ControlValidator<NextifyCheckedListBox>(null);
            }
        }

        public IControlValidator<NextifyCheckedListBox> Validator { get; set; }
        ...
        private void NextifyCheckedListBox_SelectedValueChanged(object sender, EventArgs e)
        {
            if (DesignMode)
                return;
